...
class EventDataBase():
    ...
    def _create_tables(self, df_names: pd.DataFrame,
        ...
        with self.engine.begin() as conn:
            conn.execute(Insert(self.periods),
                         [{
                            'id_period': 1, 
                            'period_name': '2021-2022', 
                            'start_date': pd.Timestamp(year=2021,
                                                        month=7,
                                                        day=1),
                            'end_date': pd.Timestamp(year=2022,
                                                     month=6,
                                                     day=30)
                         },
                         {
                            ...
                            'start_date': pd.Timestamp(year=2022,
                                                        month=7,
                                                        day=1),
                            'end_date': pd.Timestamp(year=2023,
                                                     month=6,
                                                     day=30)
                         }])